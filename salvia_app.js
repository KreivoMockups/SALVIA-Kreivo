...
function toggleKoboMenu() {
    const menu = document.getElementById('kobo-sections-menu');
    if (menu.classList.contains('hidden')) {
        menu.classList.remove('hidden');
        menu.classList.add('grid', 'grid-cols-2', 'md:grid-cols-3');
    } else {
        menu.classList.add('hidden');
        menu.classList.remove('grid', 'grid-cols-2', 'md:grid-cols-3');
    }
}

function abrirSeccionKobo(sectionId) {
    // 1. Preparamos el formulario completo de kobo
    prepararFormulario('kobo');
    
    // 2. Damos un respiro mínimo de 150ms para que el DOM (y el DAG) terminen de inyectar los inputs HTML
    setTimeout(() => {
        // Buscamos el div contenedor específico de la sección a la que queremos ir
        // Recordando que renderizarFormularioDinamico usa el ID: wrapper_eav-public-container_CODIGO
        const seccionElement = document.getElementById(`wrapper_eav-public-container_${sectionId}`);
        
        if (seccionElement) {
            // Hacemos scroll suave hasta la sección
            seccionElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
            
            // Añadimos un feedback visual para que el usuario sepa dónde aterrizó
            seccionElement.classList.add('ring-4', 'ring-[#26a69a]', 'rounded-lg', 'transition-all', 'duration-500');
            
            // Quitamos el feedback después de 2 segundos
            setTimeout(() => {
                seccionElement.classList.remove('ring-4', 'ring-[#26a69a]');
            }, 2000);
        }
    }, 150);
}
